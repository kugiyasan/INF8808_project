...
const getHeatmapCorrelation = (dataset: Entry[]): { data: HeatmapData; genres: string[] } => {
    const spokeLabels = [
      "popularity",
      "danceability",
      "energy",
    ...
    for (const [genre, group] of Object.entries(groupByGenre)) {
      ...
      for (const label of spokeLabels) {
        ...
        const covariance = d3.mean(popularityValues.map((popularity, i) => (popularity - meanPopularity) * (featureValues[i] - meanFeature)))!;
        const correlation = stdDevPopularity && stdDevFeature ? covariance / (stdDevPopularity * stdDevFeature) : 0;
