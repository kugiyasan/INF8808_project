...
const ScatterPlot: React.FC<ScatterPlotProps> = ({ data, xAxis, yAxis, selectedGenres }) => {
  ...
  useEffect(() => {
    if (svgRef.current) {
      ...
      const x = d3.scaleLinear()
        .domain([0, d3.max(filteredData, d => d[xAxis as keyof Entry] as number)!])
        .range([0, width]);

      const y = d3.scaleLinear()
        .domain([0, d3.max(filteredData, d => d[yAxis as keyof Entry] as number)!])
        .range([height, 0]);
      ...
      if (legendRef.current) {
        ...
        const genres = selectedGenres.length > 0 ? selectedGenres : colorScale.domain();
